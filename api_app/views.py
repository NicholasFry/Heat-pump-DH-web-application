...
class RunSimulation(APIView):
    def get(self, request, pk, format=None):
        ...
        nw.save('heat_pump_water', max_iter=4)#added in max iterations because of the 30sec limit on Heroku app processing times. May result in errant eta_s solutions...
        # document_model(nw, filename='report_water_design.tex', fmt=fmt)#output network model to latex report
        # offdesign test
        nw.solve('offdesign', design_path='heat_pump_water', max_iter=4)#solve the offdesign values for the network (other projected outcomes)
